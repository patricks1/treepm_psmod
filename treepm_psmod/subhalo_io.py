...
class ElvisClass(ut.io.SayClass):
    ...
    def read_tree(self, halo_directory, sub_all=None, offset_num=0):
        ...
        for zi in range(sub['id'].shape[1]):
        ...
        for si in range(sub['redshift'].shape[0]):
...
class SubhaloPropClass(ut.io.SayClass):
    ...
    def pickle_prop(self, direction='read', sub=None, zi_now=1, zi_max=15, prop='dist.cen.int',
        ...
        '''
        if not zis_assign:
            zis_assign = list(range(zi_now, zi_max + 1))
    ...
    def pickle_first_infall_znow(self, direction='read', subz=None, m_kind='m.max',
        ...
        if direction == 'write':
            ...
            subz[inf_name + '.zi'] = np.zeros(subz[list(subz.keys())[0]].size, int32) - 1
            subz[inf_name + '.zi'][sis] = inf_zis
            subz[inf_name + '.i'] = ut.array.initialize_array(subz[list(subz.keys())[0]].size)
...
def write_subhalo_mock(subz, m_kind='m.star', m_lim=[8.5, Inf], scat=0.15, dis_mf=0.007):
    ...
    print('# box length = %d Mpc' % subz.info['box.length'], file=file_out)
    print('# snapshot = %d' % subz.snap['i'], file=file_out)
    print('# redshift = %.3f' % subz.snap['z'], file=file_out)
    print('# %s = [%.2f, %.2f]' % (m_kind, m_lim[0], ms.max()), file=file_out)
    print('# scatter in log M_star - log M_max = %.2f dex' % scat, file=file_out)
    print((
        '# position(x,y,z){Mpc comoving} velocity(x,y,z){km/s physical} logM_star{M_sun / h^2} ' +
        'logM_max{M_sun} id_subhalo'), file=file_out)
    for sii, si in enumerate(sis):
        print((
            '%.4f %.4f %.4f %.2f %.2f %.2f %.3f %.3f %d') % (
            poss[sii, 0], poss[sii, 1], poss[sii, 2], vels[sii, 0], vels[sii, 1], vels[sii, 2],
            ms[si], subz['m.max'][si], si), file=file_out)
    file_out.close()
...
def write_subhalo_mock_alis(subz, halz, m_kind='m.max', m_lim=[11, Inf], dis_mf=0.007):
    ...
    print(('# masses in log{M/M_sun}, using h = %.2f' % subz.Cosmo['hubble']), file=file_out)
    print('# positions in {Mpc comoving}, velocities in {Mpc/Gyr comoving}', file=file_out)
    print('# number of subhalos = %d' % sis.size, file=file_out)
    print('# log(M_max / M_sun) = [%.2f, %.2f]' % (m_lim[0], subz[m_kind][sis].max()), file=file_out)
    print(('# id-of-subhalo id-of-central id-of-halo M_max am-sat M_halo C_200c ' +
                        'position(x,y,z) velocity(x,y,z) M_star(0) M_star(0.15) M_star(0.2)'), file=file_out)
    for sii, si in enumerate(sis):
        if subz['ilk'][si] <= 0:
            am_sat = 1
        else:
...
def write_subhalo_mgrow_cen(sub, zi_now=1, zi_max=22, m_kind='m.max', m_lim=[11, Inf],
    ...
    print('# box length = %.1f Mpc' % sub.info['box.length'], file=file_out)
    print('# snapshot = %d' % zi_now, file=file_out)
    print('# redshift = %.2f' % sub.snap['z'][zi_now], file=file_out)
    print('# age = %.3f Gyr' % sub.snap['t'][zi_now], file=file_out)
    print('# %s range = [%.2f, %.2f]' % (m_kind, m_lim[0],
                                                      sub[zi_now][m_kind][sis].max()), file=file_out)
    print((
    ...
    for sii, si in enumerate(sis):
        ...
        print((
            ...
            ms[0], ms[1], ms[2], ms[3], ms[4], ms[5], ms[6], ms[7], ms[8], ms[9]), file=file_out)
    file_out.close()
